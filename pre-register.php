...
// Handle pre-registration creation
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt->execute([
                ...
                $host_department, $qr_data, $session['id']
            ]);

            logActivity($db, $session['id'], 'pre_registration', "Created pre-registration for: $full_name", $_SERVER['REMOTE_ADDR'], $_SERVER['HTTP_USER_AGENT']);
    ...
    if (in_array($status, ['approved', 'rejected'])) {
        try {
            $stmt = $db->prepare("
                ...
                WHERE id = ?
            ");
            $stmt->execute([$status, $session['id'], $reg_id]);
            ...
            if ($pre_reg) {
                logActivity($db, $session['id'], 'pre_registration_' . $status, "Pre-registration {$status}: {$pre_reg['full_name']}", $_SERVER['REMOTE_ADDR'], $_SERVER['HTTP_USER_AGENT']);
            }
            ...
            <div class="flex justify-between h-16">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        ...
                        <p class="text-sm text-gray-500">Schedule future visits</p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <form method="POST" class="space-y-6">
                        ...
                        <div class="flex justify-end space-x-4">
                            <a href="pre-register.php" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">
                                Cancel
                            </a>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
                    <form method="GET" class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4">
                        ...
                        <select name="status" class="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                        ...
                        <input type="date" name="date" value="<?php echo htmlspecialchars($date_filter); ?>" 
                               class="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                        <button type="submit" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg font-medium transition-colors">
                            <i class="fas fa-search mr-2"></i>Search
                        </button>
                    </form>
                    
                    <a href="pre-register.php?action=create" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-medium transition-colors inline-flex items-center">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Host Info</th>
